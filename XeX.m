...
function XeX_OpeningFcn(hObject, eventdata, handles, varargin)
	...
	handles.XeXDir=fileparts(which('XeX'));
	if isempty(handles.XeXDir)
	...
	for TemporaryVariable=1:handles.NumberOfAxes
		handles.HandlesList(TemporaryVariable) = handles.(['axes' num2str(TemporaryVariable)]);
		handles.EditWindows(TemporaryVariable) = handles.(['AxisObj' num2str(TemporaryVariable)]);
		handles.AxisLinkTexts(TemporaryVariable) = handles.(['AxisLinkText' num2str(TemporaryVariable)]);
	end
	...
	if strcmp(get(handles.ShowAllObjWin,'checked'),'on')
		...
		makeInvisible(handles.EditWindows);
	end
	makeInvisible([handles.AxisLinkTexts]);
	handles = LayOutEditWindow(handles);
	...
	% Other stuff - not categorizing for now since some of it is duplicated, etc.
...
function LoadedFiles_Callback(hObject, eventdata, handles)
	...
	if ~iscell(LoadedFileList)
		TempVar{1}=LoadedFileList;
		LoadedFileList=TempVar;
	end
	...
	if ~strcmp(CurrentDataFile,'LoadedFiles')
		...
		if isfield(handles.(CurrentDataFile),'Analysis')
			...
			MyResponseVariableList=handles.(CurrentDataFile).Analysis.MyResponseVariableList;
			CurrentVariables=handles.(CurrentDataFile).Analysis;
			fprintf('%s\n',CurrentVariables.summaryString);
	...
	if ~strcmp(CurrentlyLoadedFiles{1},'LoadedFiles')
		for FileIndex=1:length(CurrentlyLoadedFiles)
			...
			if isnumeric(TempStore) && isnan(TempStore)
				...
				if length(JustLoadedTrials)>0
					eval([CurrentFile ' .Trials=JustLoadedTrials;']);
					clear JustLoadedTrials;
				end				
			else
...
function PlotData_Callback(hObject, eventdata, handles)
	...
	elseif strcmp(WhatToDo,'MakePlot')
		...
		NextCommandSetBaseRate=0;
		if isempty(CurrentUnitNumber)
			CurrentUnitNumber=613;
			fprintf('%s\n','Using 613 as the Unit');
		end
		if ~strcmp(CurrentDataFile,'LoadedFiles') & ~strcmp(CurrentPlottingFile,'PlottingFile') 
			for numFilters=1:length(FilterStringList)
				...
				if isempty(CurrentFilterString)
...
function LoadFile_Callback(hObject, eventdata, handles)
	...
	handles = LoadFileCallbackStuff(handles);
	guidata(hObject,handles);
end
...
function FTPOptions_Callback(hObject, eventdata, handles)
	...
	if ~isfield(handles,'FTP')
		...
		CurrentFTPPath=(handles.FTP.pathname);
...
function PlottingFileList_Callback(hObject, eventdata, handles)
	...
	switch(currentselection)
		case 'ImageMap.m'
			set(handles.StimulusAlignedTimeVector,'string','[50 150]');
			if isempty(char(get(handles.TrialFilterVector,'string')))
				set(handles.TrialFilterVector,'string','f==1&PhotoBad~=1');
			end
			XAxisVariables=get(handles.XAxisVariable,'string');
			WhereisRFx=find(strcmp(XAxisVariables,'RFx'));
			if ~isempty(WhereisRFx)
				set(handles.XAxisVariable,'value',WhereisRFx(1));
				set(handles.XAxisDiscretize,'value',1);
				set(handles.XAxisCenters,'string','-25:5:25');
			end
			X1AxisVariables=get(handles.X1AxisVariable,'string');
			WhereisRFy=find(strcmp(X1AxisVariables,'RFy'));
			if ~isempty(WhereisRFy)
				set(handles.X1AxisVariable,'value',WhereisRFy(1));
				set(handles.X1AxisDiscretize,'value',1);
				set(handles.X1AxisCenters,'string','-25:5:25');
			end
			YAxisVariables=get(handles.YAxisVariable,'string');
			WhereisSpikeRate=find(strcmp(YAxisVariables,'SpikeRate'));
			if ~isempty(WhereisSpikeRate)
				set(handles.YAxisVariable,'value',WhereisSpikeRate(1));
			end
		case 'CircleMap.m'
			set(handles.StimulusAlignedTimeVector,'string','[50 150]');
			if isempty(char(get(handles.TrialFilterVector,'string')))
				set(handles.TrialFilterVector,'string','GoodTrial==1&PhotoBad~=1');
			end
		case 'SpikeDensity.m'
			set(handles.StimulusAlignedTimeVector,'string','[-400 400]');
		case 'DrawPSTH.m'
			set(handles.StimulusAlignedTimeVector,'string','[-400 400]');
	end
...
function AddAnalysisFileButton_Callback(hObject, eventdata, handles)
	...
	if ~exist('DataDirectory','var')
		DataDirectory='c:\MATLAB\work\XeX\PreProcFunctions';
	end
	cd(DataDirectory);
...
function DeleteLoadedFiles_Callback(hObject, eventdata, handles)
	...
	if ~iscell(LoadedFileList) && strcmp(LoadedFileList,'LoadedFiles')
		...
		if isempty(FilesToKeep)
			LoadedFileList='LoadedFiles';
		else
			LoadedFileList=LoadedFileList(FilesToKeep);
		end
...
function ExploreVariables_Callback(hObject, eventdata, handles)
	...
	if ~strcmp(CurrentDataFile,'LoadedFiles')
		...
		eval(['if GoOn==1, CurrentVariables=handles.' CurrentDataFile '.Analysis;end']);
		detachvars;
		if exist('CurrentVariables','var')
			clear CurrentVariables;
		end  %***newly added to clear off possible big structure from workspace.
		keyboard;
		...
		if CurrentAxis~=NewAxis
			...
			if exist('CurrentObjects','var') && length(CurrentObjects)>=1
				...
				if max(ObjectsToCopy)<=length(CurrentObjects)
					...
					if exist('NewObjects','var')
						NewObjects=[NewObjects TempObjects(ObjectsToCopy)];
					else
						NewObjects=TempObjects(ObjectsToCopy);
					end
...
function GetObjectPropsButton_Callback(hObject, eventdata, handles)
	...
	if isnumeric(AxesToGet) && length(AxesToGet)==1
		...
		if isempty(OutputString)
			OutputString={'Nothing. Nada. Zilch. No objects here in this axis'};
		end
...
function SpeakToMeMenu_Callback(hObject, eventdata, handles)
	...
	if strcmp(get(hObject,'Checked'),'off')
		set(hObject,'checked','on'); handles.SpeakToMe=1;
	else
		set(hObject,'Checked','off'); handles.SpeakToMe=0;
	end
	guidata(hObject,handles);
end
...
function ErrorBarType_Callback(hObject, eventdata, handles)
	...
	if strcmp(get(hObject,'label'),'ChangeErrorToBootstrap')
		set(hObject,'label','ChangeErrorToPoisson');
		fprintf('%s\n','Confidence bands now Bootstrap'); 
	else
		set(hObject,'label','ChangeErrorToBootstrap');
...
function FiltersList_Callback(hObject, eventdata, handles)
	...
	if ~isappdata(0,'TrialFilterHandle')
		setappdata(0,'TrialFilterHandle',handles.TrialFilterVector);
	end
	% if ~isappdata(0,'UseFilterHandle'), setappdata(0,'UseFilterHandle',handles.UseFilters);end
	if ~isappdata(0,'FilterGUIHandle')
		FilterManagerGUI;
		if ~isappdata(0,'UsePlottingHandle')
			setappdata(0,'UsePlottingHandle',handles.UseProps);
		end
		if ~isappdata(0,'HoldFilterHandle')
			setappdata(0,'HoldFilterHandle',handles.HoldStatus);
		end
	else
		figure(getappdata(0,'FilterGUIHandle'));
	end
	guidata(hObject,handles);
end
...
function ManageObjects_Callback(hObject, eventdata, handles)
	...
	if strcmp(get(hObject,'string'), 'ManageObjects')
		set(hObject,'string','BackToVars');
		makeVisible(Objs);
		makeInvisible(VarObjs);
	else
		set(hObject,'string','ManageObjects');
		makeInvisible(Objs);
		makeVisible(VarObjs);
	end
	...
	if ~strcmp(CurrentDataFile,'LoadedFiles')
		...
		if exist(CurVar,'var')
			...
			if length(UnVar)<50
				...
				fprintf('There are %d unique variables\n',length(UnVar));
			end
			clearvars;
		else
...
function AddGrapher_Callback(hObject, eventdata, handles)
	...
	Old_Dir=pwd;
	if ~exist('GraphCommandsDirectory','var')
		GraphCommandsDirectory='c:\MATLAB\work\XeX\MyGraphs';
	end
