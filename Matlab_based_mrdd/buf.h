 ...
 */
typedef struct {
	snt filedes;	/* file descriptor for this channel */
...
typedef struct {
	...
	snt	acontinue;	/* flag: 1 if record is a continuation
				   record */
	snt	alength;	/* byte count of following data */
} ANALOGHDR;
 ...
 */
typedef struct {
	snt i_correct;	/* number of correct trials */
	snt i_wrong;	/* number of wrong trials */
...
#define	INT_BLOCK_FIRST	ptbl[0]	/* define to be first member of struct */   
	...
	snt	int_pi;		/* Indexes to ptbl for main REX procs;
	...
	snt	scrb_pi;
	snt	disp_pi;
	u_snt	c_flags;	/* system wide flags for comm process */
	u_snt	i_flags;	/* system wide flags for short process */
	u_snt	d_flags;	/* system wide flags for display process */
	snt	d_rwakecd;	/* wake up display to update rasters when
				   this ecode is loaded by state processor */
	u_snt	d_rl_disp;	/* running line disp flags */
	u_snt	d_wd_disp;	/* window disp flags */
	snt	d_emag;		/* window disp eye cursors magnification */
	snt	d_omag;		/* window disp offset cursors magnification */
	snt	i_nindex;		/* name index of proc's noun or menu
	...
	snt	i_rtflag;	/* root control flag */
	snt	i_rtcx;		/* root file current read index */
	long	i_rtseekp;	/* root file seek pointer to end of block
				   currently in buffer */
	snt	i_rtcln;	/* current line number of root file */
	...
	snt	i_errptx;	/* proc tbl index for proc generating error */
	u_snt	evlx;		/* load index for event buf;  posnts to next
				   location to load */
	u_snt	evdx;		/* points to next location to be dumped */
	u_snt	evdump;		/* event dump index;  when evlx == evdump
						   scribe will dump event buffer */
	u_snt	seqnum_a;	/* A-file sequence number */
	u_snt	seqnum_e;	/* E-file sequence number */
 ...
 */
typedef struct {
	snt	ws_flag;	/* type of entry */
	snt	ws_code;	/* ecode for this entry */
	snt	ws_cont;	/* continuation count */
	long	ws_acount;	/* acount */
	long	ws_time;	/* time for this write */
	snt	ws_lowx;	/* low and high indexes of write */
	snt	ws_highx;
} WSTACK;
 ...
 */
typedef struct {
	snt	r_rawlx;	/* load index for cir analog buffers;  points
				   to next location to load */
	snt	r_cendx;	/* current end index of rotating buffer */
	snt	r_wrapx;	/* target wrap index */
	snt	r_bufendx;	/* absolute end of rotating buffer */
	snt	r_fillcnt;	/* current undumped count of buffer;  used
				   to check for buffer overflow */
	snt	r_wslx;		/* load index of write stack;  points to next
				   location to load */
	snt	r_wsdx;		/* dump index;  points to next location to
				   dump */
	snt	r_sam_swtch;	/* controls sampling;  when set to MININT,
				   sampling is disabled */
	snt	r_bufinc;	/* index distance between same index location in
