...
if ~isempty(TemporaryVariable), RFRed(TrialIndex)=CurrentCodes(TemporaryVariable(1))-5000;
...
FirstDSacLat(TrialIndex)=SacLat(find_the_sac(1));
    ...
    find_the_sac=find(StartFromFP<4 & EndAtRF2 < 5 & SacLat>= (GapTime(TrialIndex)-FPOnTime(TrialIndex))-50);
    if length(find_the_sac)>1, fprintf('%d\t%s\n',TrialIndex,'More than one fp_to_rf2 !!');end
%     if GoodTrial(TrialIndex)==1 & length(find_the_sac)==0, keyboard; end
...
dRFLat=DiscretizeMe(RFLat,[100 150 200]);
