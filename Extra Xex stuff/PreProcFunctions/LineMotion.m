function CurrentVariables=LineMotion(Trials,klugestring,UseAx,SacOpts)
	...
	for TrialIndex=1:length(Trials)
		...
		% if ~isempty(is_fp)
		% 	FPTime(TrialIndex)=CurrentTimes(is_fp(1));
		% end
		...
		if ~isempty(is_fp)
			...
			if length(EarlyDrops)>=7
				RFLen(TrialIndex)=(CurrentCodes(EarlyDrops(7)))-11000;
			end
		end
	
		if ~isempty(is_photobad)
			PhotoBad(TrialIndex)=1;
		end
		if ~isempty(is_abort)
			AbortTrial(TrialIndex)=1;
		end
		if ~isempty(is_fixationbreak)
			FixationBreak(TrialIndex)=1;
		end

		if ~isempty(is_rf)
			RFOnTime(TrialIndex)=CurrentTimes(is_rf(1));
		...
		if ~isempty(is_rf2)
			RF2OnTime(TrialIndex)=CurrentTimes(is_rf2(1));
		...
		if ~isempty(is_rf) & GoodTrial(TrialIndex)~=1 & AbortTrial(TrialIndex)~=1 & FixationBreak(TrialIndex)~=1
			if ~isempty(is_error)
				BadTrial(TrialIndex)=1;
			else
				BadTrial(TrialIndex)=2;
			end
		end
		...
		if ~isempty(TemporaryArrayNumber)
			if length(TemporaryArrayNumber)==1
				ArrayNumber(TrialIndex)=CurrentCodes(TemporaryArrayNumber)-2000;
				ArrayBak=[];
			elseif length(TemporaryArrayNumber)==2
				ArrayNumber(TrialIndex)=CurrentCodes(TemporaryArrayNumber(1))-2000;
				ArrayBak=CurrentCodes(TemporaryArrayNumber(2))-2000;
			else
				ArrayNumber(TrialIndex)=-1;
				ArrayBak=[];
			end
		elseif ~isempty(ArrayBak)
			ArrayNumber(TrialIndex)=ArrayBak;
			ArrayBak=[];
		end
	end
